# Filing package — the logo

Everything needed to file, plus one thing that must be fixed first.
...
A drawing filed **without a colour claim** protects the mark in **any** colour.
Claim colour and you are limited to that scheme.

This matters here specifically: the mark is used navy-on-cream *and* reversed
cream-on-navy on dark garments. A black-and-white drawing covers both. A colour
claim would cover one and leave the other exposed. **File black and white unless
an attorney says otherwise.**

Both drawings meet the USPTO requirements: `.jpg`, 250–944 px on each side,
300 dpi, RGB, clean high-contrast lines, minimal white space around the design.

---

## Pre-filled TEAS fields

| Field | Value |
|---|---|
| Mark type | **Special form** (design / stylised) |
| Colour claim | **No** — file `drawing-bw.jpg` |
...
| Date of first use in commerce | See below |
...
public use is **29 March 2026** — the first commit, "Launch The Sports Page —
Issues 1 & 2".

Overstating a first-use date is a known way to get a registration challenged.
**Claim 29 March 2026**, which is documented, hash-chained, and continuous — or
have the attorney decide whether the 27th is independently provable.

Supporting record: 442 commits, 140 published issues, unbroken 29 March to
11 August 2026. Most small applicants cannot evidence first and continuous use
...
- **A professional clearance search.** Neither search run so far was one — see
  `corpus/trademark-prep.md` §2. The USPTO search API was unreachable and Justia
  blocks automated access, so both relied on secondary sources.
- **A decision on Class 25.** Shirts are in production. Clothing is a separate
  class, it collides with the non-commercial licence, and it puts you nearer the
  existing "Sports Page" bar and restaurant marks. Decide before filing, not after.
- **An attorney.** Particularly on the descriptiveness question, which determines
  whether this is a design-mark filing, a composite with a disclaimer, or the
  Supplemental Register.

---

*I cannot submit the application. A USPTO filing needs the applicant's signature
under penalty of perjury, the applicant's payment, and either the applicant
personally or a licensed attorney as filer. This package is built so that filing
takes minutes once those three things are in place.*
