...
11. **Commit and push**:
    ```
    git add index.html published/FILENAME.html QUEUE_ORDER.txt AGENTS.md
...
**When creating new queue files**, always include PUBLISH-META. Codex should add this automatically when generating new issues.
...
- **BOTTOM LINE UP FRONT, AND WRITE IT PLAIN.** This is the second rule because a
  ...
  **Plain is not dumb, and plain is not soulless.** The `humanizer` skill
  (`~/.Codex/skills/humanizer/SKILL.md`) is the reference for voice: vary
...
The newsletter exists in time. Each month has a different mix of what is actually happening in the sports world, and good editorial judgment matches today's issue to today's news cycle. This calendar tells Codex what is "live" and what is dormant in any given month, plus which annual events warrant pre-planned special treatment.
